# -*- coding: utf-8 -*-
"""실시간 시세 — 10분마다 갱신해 live.enc를 굽는다.

무거운 잡(update_data.py, AI·KB시세·배당·이력)과 분리한 가벼운 잡.
블록마다 수명(TTL_MIN)이 달라 만료된 것만 다시 읽으므로, 주기를 10분으로 당겨도
매 실행 시트 왕복은 T1 두 블록뿐이다.

담는 것: 지수·환율(yfinance) + 계좌 평가액(수량은 시트, 가격은 quotes.py가 직접)
소요: 시트 2~3콜 + yfinance 2콜 ≈ 30초
...
※ 무료로 실시간 시세를 주는 곳은 없다. 정직하게 적어 둔다.
  · 미국 ETF/주식 : Yahoo 사실상 실시간 → 10분 주기면 화면은 ~10분 이내
  · 국내 ETF(.KS) : Yahoo도 15~20분 지연 → 화면은 ~25~30분
  종전에는 시트 GOOGLEFINANCE(15~20분 지연) + 시트 재계산 시점 불확실 + 30분 주기라
  최대 ~50분 전 값이었다. quotes.py가 가격을 직접 받아 그 사슬을 끊었다.
"""
import hashlib
import json
import os
...
TTL_MIN = {
    # ── T1 실시간 (목표 ≤10분) — 매 실행 ──────────────────────────────────
    "financial":     0,     # 계좌 평가액. 가격은 quotes.py가 직접 받아 곱한다
    "checkpoint":    0,     # 셀 1개라 저렴 + 모닝 리포트를 늦추지 않으려고
    # ── T2 반나절 (≤3시간) — 사람이 시트를 고치면 그 안에 반영된다 ─────────
    "fire":          180,   # ★종합 목표치. 달성률은 프론트가 순자산으로 재계산
    "simulation":    180,   # 은퇴 지도 — 경기순환 설정을 바꾸면 3시간 내
    "spending":      180,   # 가계부
    "annual_flow":   180,   # 연도별 소득·지출
    "asset_history": 180,
    "liabilities":   180,
    "monthly":       180,
    "baselines":     180,   # 전일·전월·전년 기준 — 달 바뀜을 놓치지 않게
    # ── T3 반일 (240분) — 바깥이 하루 몇 번 바뀌는 것 ─────────────────────
    "officetel":     240,   # 국토부 실거래 신고가 수시로 올라온다(API 24콜)
    # ── T4 주 단위 (720분) — 사람이 손댈 때만 바뀐다 ──────────────────────
    "cities":        720,   # 🌍이주 대시보드 — 도시를 더할 때만
    "review":        720,   # 연간 리뷰
}
# 등급을 나누는 기준은 '얼마나 자주 바뀌는가'가 아니라 **'얼마나 어긋나도 되는가'**다.
# 부동산 KB시세는 주 1회(금)만 바뀌므로 일별 잡(update.yml)이 맡고 여기엔 없다.
# 값이 아니라 '읽기'를 아끼는 것이라 페이로드는 언제나 완전하다.
#
# officetel만 240분인 이유 — 다른 블록은 시트를 읽을 뿐이지만 이 블록은 국토부 실거래
# API를 24콜(2개 지역 × 12개월) 두드린다. 10분마다면 하루 3,456콜로 개발계정 한도의
# 3분의 1을 태운다. 4시간이면 하루 6회 · 144콜(1.5%)이고, 신고 반영도 충분히 빠르다.
...
def main():
    ...
    try:
        ...
        financial = None
    fin = financial or {}
    # 시트의 평가액은 GOOGLEFINANCE 계산값이라 15~20분 지연이고, 시트가 언제 재계산
    # 되는지는 우리가 못 정한다. 티커·수량은 시트에서 받고 가격만 우리가 받아 곱한다.
    # 시세를 못 받으면 아무것도 바꾸지 않는다(시트 값 유지).
    if fin.get("holdings"):
        try:
            import quotes
            fin = quotes.apply_live_prices(fin)
        except Exception as e:  # noqa: BLE001 — 시세 실패가 발행을 막지 않는다
            print(f"WARN: 평가액 재계산 실패 ({type(e).__name__}: {e}) — 시트 값 유지")
    accounts, total = {}, 0
    ...
    # 3) 관심 뉴스 — 매 실행 최신으로. 실패해도 시세는 나가야 하니 []로 폴백.
    try:
