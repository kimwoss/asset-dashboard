...
집계 범위(2026-08 사용자 확정): 고정비+변동비 전부, 단 '기타'만 제외.
이 지표는 '한 달에 통장에서 얼마가 나가나'다 — 대출 상환도 실제로 나가는 돈이라 넣는다.
'기타'만 빼는 건 소비가 아니어서가 아니라, 전세보증금 같은 대형 자금이동이 그 칸에
섞여 들어와 월평균을 통째로 무너뜨리기 때문이다. 뺀 값은 버리지 않고 excluded로 넘겨
표 하단에서 대조할 수 있게 한다.
"""
...
EXCLUDED = {
    # '대출'은 뺐다가 다시 넣었다 — 매달 통장에서 실제로 나가는 돈이라, 빼면 월별 지출이
    # 온전하지 않다(2026-08-09 사용자 요청). 원금이 섞여 있어 순수 소비는 아니지만,
    # 여기 지표는 '한 달에 얼마가 나가나'이지 '얼마를 소비했나'가 아니다.
    # (연도별 저축률은 성격이 달라 시각화 탭에서 따로 다룬다.)
