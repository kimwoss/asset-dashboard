...
function renderOfficetel(of) {
  ...
  const live = of.items.filter(x => x.ok);
  ...
  // ── 정렬 ────────────────────────────────────────────────────────────────
  // 기본은 별점 높은 순 → 같으면 싼 순(중개사가 훑는 순서). 전용·환산전세·확인일
  // 머리글을 누르면 그 열로 정렬하고, 같은 열을 다시 누르면 방향이 뒤집힌다.
  // 값이 빈 행(전용 미상 등)은 방향과 무관하게 늘 아래로 보낸다 — 빈 칸이 위에
  // 몰리면 정렬한 보람이 없다.
  const OF_SORT = {
    area:      { get: x => x.area,       dir:  1 },   // 작은 평형부터
    converted: { get: x => x.converted,  dir:  1 },   // 싼 것부터
    asof:      { get: x => x.asof || "", dir: -1 },   // 최신부터
  };
  let ofSort = null;

  const ofSorted = () => {
    if (!ofSort) return [...live].sort((p1, p2) =>
      (p2.stars || "").length - (p1.stars || "").length || p1.converted - p2.converted);
    const get = OF_SORT[ofSort.key].get, d = ofSort.dir;
    return [...live].sort((p1, p2) => {
      const x = get(p1), y = get(p2);
  ...
  const ofTh = (key, label) => {
    const on = ofSort && ofSort.key === key;
    return '<th class="of-sort' + (on ? " on" : "") + '" data-sort="' + key + '"'
         + ' role="button" tabindex="0" title="눌러서 정렬" aria-sort="'
         + (on ? (ofSort.dir > 0 ? "ascending" : "descending") : "none") + '">'
         + label + '<span class="of-arrow">' + (on ? (ofSort.dir > 0 ? "▲" : "▼") : "⇅") + "</span></th>";
  };

  function paintLive() {
    const rows = ofSorted();
    document.getElementById("of-live").innerHTML =
      "<thead><tr><th>\uc624\ud53c\uc2a4\ud154</th><th>\ud0c0\uc785</th>"
      + ofTh("area", "전용") + "<th>구조</th>"
      + "<th>보증금/월세</th>" + ofTh("converted", "환산전세")
      + "<th>층·향</th>" + ofTh("asof", "확인일") + "<th></th></tr></thead><tbody>"
  ...
  const ofTable = document.getElementById("of-live");
  const ofPick = key => {
    if (!OF_SORT[key]) return;
    ofSort = (ofSort && ofSort.key === key) ? { key, dir: -ofSort.dir }
                                            : { key, dir: OF_SORT[key].dir };
    paintLive();
  };
  ofTable.addEventListener("click", ev => {
    const th = ev.target.closest("th.of-sort"); if (th) ofPick(th.dataset.sort);
  });
  ofTable.addEventListener("keydown", ev => {
    if (ev.key !== "Enter" && ev.key !== " ") return;
    const th = ev.target.closest("th.of-sort");
    if (th) { ev.preventDefault(); ofPick(th.dataset.sort); }
  });
